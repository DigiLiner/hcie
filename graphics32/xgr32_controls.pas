...
type
  ...
  public
    ...
    procedure PaintChildsTo(aBuffer: TBitmap32; const aPaintAll: boolean = False);
      overload;
    ...
    procedure PaintChildsTo(aBuffer: TBitmap32; const srcRect: TRect;
      const aFirstControl: integer = 0; const aPaintAll: boolean = False); overload;
    ...
    function PaintSelfToBuffer(aBuffer: TBitmap32;
      const aPaintAll: boolean = False): boolean; overload;
    ...
    property StyleController: TGRStyleController
      read FStyleController write SetStyleController;
  ...
  protected
    ...
    {$ENDIF}
    property ActiveTextColor: TColor read GetActiveTextColor
      write FActiveTextColor default clNone;
    property CaptionFont: TFont32 read FCaptionFont write SetCaptionFont;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    ...
    procedure PaintSelfTo(aBuffer: TBitmap32; DstRect, srcRect: TRect); overload;
    { Summary Paint the Component Foreground itself(No parent background, or
      sons). }
    { Description
    Φ»»ζΦΖΗ°Ύ°£¨²»°όΐ¨ΛόµΔΈΈ±³Ύ°ΊΝΛόΟΒΚτµΔΧΣΤΌώ£©
    ...
    function PaintSelfToBuffer(aBuffer: TBitmap32; const srcRect: TRect): boolean;
      overload;
    { Summary Paint itself to DC }
    { Description
    srcRect: witch to paint. ±νΓχΠθ»ζΦΖΈΓΤΌώµΔΔΗ»²Ώ·ΦµΔΗψΣς΅£
  ...
  begin
    ...
    try
      if aControl.Parent is TGRCustomControl then
        ...
        with aControl.Parent as TWinControl do
        begin
          P := ClientOrigin;
          ScreenToClient(P);
          MoveWindowOrg(DC, -P.X, -P.Y);
...
begin
  ...
  if (aChild is TGRGraphicControl) and not ((csLoading in ComponentState) and
...
begin
  ...
  begin
    LIsDragging := IsMouseButtonDown(TMyShiftStates(Shift), ssLeft) and
      IsMouseButtonDown(TMyShiftStates(FShiftState), ssLeft) and
      ((Abs(X - FMouseXPos) >= 2) or (Abs(Y - FMouseYPos) >= 2));
...
procedure TGRCustomControl.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: integer);
begin
  FShiftState := Shift;
...
procedure TGRCustomControl.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TGRCustomControl.PaintChildsTo(aBuffer: TBitmap32;
  const aPaintAll: boolean = False);
var
  srcRect: TRect;
begin
  srcRect := ClientRect;
...
procedure TGRCustomControl.PaintChildsTo(aBuffer: TBitmap32;
  const srcRect: TRect; const aFirstControl: integer = 0;
  const aPaintAll: boolean = False);
var
  I: integer;
  aControl: TControl;
...
begin
  ...
  begin
    ...
    begin
      aControl := Controls[i];
      if not aControl.Visible or not GR32.IntersectRect(aRect, srcRect,
        aControl.BoundsRect) then continue;
...
procedure TGRCustomControl.PaintSelfTo(aBuffer: TBitmap32; const srcRect: TRect);
begin
  if FSelfBufferDirty then
  begin
...
function TGRCustomControl.PaintSelfToBuffer(aBuffer: TBitmap32;
  const aPaintAll: boolean = False): boolean;
begin
...
function TGRCustomControl.PaintSelfToBuffer(aBuffer: TBitmap32;
  const srcRect: TRect; const aPaintAll: boolean = False): boolean;
begin
  aBuffer.SetSize(Width, Height);
  aBuffer.Clear(0);
  aBuffer.Font := FCaptionFont;
...
not FFastDraw and
  {$ENDIF}
  (Message.DC <> 0) and HandleAllocated then
  begin
...
procedure TGRGraphicControl.PaintSelfTo(aBuffer: TBitmap32; DstRect, srcRect: TRect);
var
  I: integer;
  aControl: TControl;
  aBMP: TBitmap32;
...
function TGRGraphicControl.PaintSelfToBuffer(aBuffer: TBitmap32;
  const srcRect: TRect): boolean;
begin
  aBuffer.SetSize(Width, Height);
  aBuffer.Clear(0);
  aBuffer.Font := FCaptionFont;
...
begin
  ...
  if Transparent or FAlphaBlend then
    StretchToDCTransparentFunc(DC, SrcRect.Left, SrcRect.Top,
