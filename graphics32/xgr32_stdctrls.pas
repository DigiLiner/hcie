...
uses
  LCLIntf, LCLType,
  {$IFDEF WINDOWS}  Windows, CommCtrl  ,{$ENDIF}
...
type
  ...
  protected
    ...
    property Checked: boolean
      read GetChecked write SetChecked stored IsCheckedStored default False;
  ...
  private
    ...
    FShowGlyph: boolean;
    procedure CalcButtonLayout(bmp: TBitmap32; const Client: TRect;
      const Offset: TPoint; const aCaption: string; Layout: TGRButtonLayout;
      Margin, Spacing: integer; var GlyphPos: TPoint; var TextBounds: TRect;
      BiDiFlags: longint);
    procedure DrawButtonGlyph(bmp: TBitmap32; const GlyphPos: TPoint;
      State: TGRButtonState; Transparent: boolean);
    procedure DrawButtonText(bmp: TBitmap32; const aFont: TFont32;
      const aCaption: string; TextBounds: TRect; State: TGRButtonState;
      BiDiFlags: longint);
  ...
  protected
    ...
    procedure CreateParams(var Params: TCreateParams); override;
    function InternalPaintBuffer(aBitmap32: TBitmap32): boolean; overload; override;
  public
    constructor Create(AOwner: TComponent); override;
  end;
  ...
  protected
    ...
    procedure DoDrawText(Dst: TBitmap32; var Rect: TRect; Flags: longint); dynamic;
...
type
  ...
  private
    ...
    procedure DrawButtonGlyph(bmp: TBitmap32; const GlyphPos: TPoint;
      State: TGRButtonState; Transparent: boolean);
    procedure DrawButtonText(bmp: TBitmap32; const aFont: TFont32;
      const Caption: string; TextBounds: TRect; State: TGRButtonState;
      BiDiFlags: longint);
    procedure GlyphChanged(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      if (FState in [bsDown, bsExclusive]) or (FMouseInControl and
        (FState <> bsDisabled)) or (csDesigning in ComponentState) then
      begin
...
PWideChar
      ...
      PChar
      {$ENDIF}
      (Caption), Length(Caption), TextBounds,
...
function TButtonGlyph.Draw(bmp: TBitmap32; const aFont: TFont32;
  const Client: TRect; const Offset: TPoint; const Caption: string;
  Layout: TGRButtonLayout; Margin, Spacing: integer; State: TGRButtonState;
  Transparent: boolean; BiDiFlags: longint): TRect;
var
  GlyphPos: TPoint;
begin
...
procedure TButtonGlyph.DrawButtonText(bmp: TBitmap32; const aFont: TFont32;
  const Caption: string; TextBounds: TRect; State: TGRButtonState; BiDiFlags: longint);
var
  bakEnabled: boolean;
begin
  //with bmp.Canvas do
...
begin
  ...
  with Params do
  begin
    Style := Style or BS_3STATE or Alignments[UseRightToLeftAlignment, FAlignment];
...
begin
  ...
  CreateSubClass(Params, 'BUTTON');
  with Params do
    Style := Style or BS_RADIOBUTTON or Alignments[UseRightToLeftAlignment,
      FAlignment];
end;
...
procedure TGRCustomLabel.DoDrawText(Dst: TBitmap32; var Rect: TRect; Flags: longint);
var
  Text: string;
  bakColor: TColor;
  bakEnabled: boolean;
...
procedure TGRCustomLabel.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TGRButtonControl.ActionChange(Sender: TObject; CheckDefaults: boolean);
begin
  inherited ActionChange(Sender, CheckDefaults);
...
procedure TGRCustomButton.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: integer);
begin
...
begin
  ...
  NeedRepaint := FMouseInControl and Enabled and (DragMode <> dmAutomatic) and
    (GetCapture = 0);
...
procedure TGRButton.CalcButtonLayout(bmp: TBitmap32; const Client: TRect;
  const Offset: TPoint; const aCaption: string; Layout: TGRButtonLayout;
  Margin, Spacing: integer; var GlyphPos: TPoint; var TextBounds: TRect;
  BiDiFlags: longint);
var
...
PWideChar
      ...
      PChar
      {$ENDIF}
      (aCaption), Length(aCaption), TextBounds,
      DT_CALCRECT or BiDiFlags);
    TextSize := Point(TextBounds.Right - TextBounds.Left, TextBounds.Bottom -
      TextBounds.Top);
  end
...
begin
  ...
  FMouseInControl := (LPoint.X >= 0) and (LPoint.X <= Width) and
...
procedure TGRButton.DrawButtonText(bmp: TBitmap32; const aFont: TFont32;
  const aCaption: string; TextBounds: TRect; State: TGRButtonState; BiDiFlags: longint);
var
  bakEnabled: boolean;
begin
...
begin
  Result := (FClient is TGRCustomButton) and (FClient.GroupIndex =
    (Action as TCustomAction).GroupIndex);
end;
